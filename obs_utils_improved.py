...
def interactive_mode():
    ...
    try:
        ...
        operation = get_user_input("Select operation [l/a/i/r/d/f/x]: ", required=True).lower()
        ...
        if operation == "x":
            bucket = get_user_input("Enter bucket name (press ENTER to search all buckets): ", required=False)
        ...
        if operation == "l":
            ...
            days = get_user_input("Enter restore days (default: 30): ", required=False, default="30")
            tier = get_user_input(
                "Enter restore tier [Expedited/Standard/Bulk] (default: Expedited): ", required=False, default="Expedited"
            )
            ...
            download_path = get_user_input("Enter local download path (press ENTER for default): ", required=False)
        ...
        create_sample = get_user_input(
            "Would you like to create a sample configuration file? [y/N]: ", required=False, default="n"
        ).lower()
...
def command_line_mode(args):
    ...
    try:
        ...
        if args.operation == "list":
            ...
            if args.object_key:
                success = obs_manager.download_single_file(args.bucket, args.object_key, args.download_path)
                count = 1 if success else 0
            else:
                count = obs_manager.download_objects(args.bucket, args.prefix or "", args.download_path)
...
Examples:
  ...
  python obs_utils_improved.py --operation download --bucket my-bucket --object-key file.txt
    ...
    parser.add_argument("--config", default="obs_config.json", help="Configuration file path (default: obs_config.json)")

    parser.add_argument(
        "--operation", choices=["list", "archive", "warm", "restore", "download", "search"], help="Operation to perform"
    )
    ...
    parser.add_argument("--days", type=int, default=30, help="Number of days for restore operation (default: 30)")

    parser.add_argument(
        "--tier", choices=["Expedited", "Standard", "Bulk"], default="Expedited", help="Restore tier (default: Expedited)"
    )

    parser.add_argument("--create-config", action="store_true", help="Create sample configuration file and exit")

    parser.add_argument("--setup-secure-config", action="store_true", help="Run interactive secure configuration setup")

    parser.add_argument("--encrypt-config", action="store_true", help="Encrypt existing configuration file")

    parser.add_argument("--secure-permissions", action="store_true", help="Set secure permissions on configuration file")

    parser.add_argument("--setup-security-levels", action="store_true", help="Setup multi-level security system")

    parser.add_argument("--list-security-levels", action="store_true", help="List configured security levels")

    parser.add_argument("--enable-security-levels", action="store_true", help="Enable multi-level security for this session")

    parser.add_argument("--test-mode", action="store_true", help="Run in test mode (skip credential validation for CI/CD)")
